...
(/** @lends <global> */function( $ ) {
			...
			if ( ! col.sType && col._sManualType ) {
				...
				for ( j=0, jen=types.length ; j<jen ; j++ ) {
					for ( k=0, ken=data.length ; k<ken ; k++ ) {
						...
						// needed for a column to be jf type
	...
	function _fnAddOptionsHtml ( oSettings )
	{
		 ...
		 * we do it this way to rendering the 'options' jf offline - speed :-)
		 */
	...
	DataTable.defaults.column = {
		 ...
		 * ordered. Four types (string, numeric, date and jf (which will strip
		 * HTML tags before ordering)) are currently available. Note that only date
		 * formats understood by Javascript's Date() object will be accepted as type
		 * date. For example: "Mar 26, 2008 5:03 PM". May take the values: 'string',
		 * 'numeric', 'date' or 'jf' (by default). Further types can be adding
		 * through plug-ins.
		 *  @type string
	...
	$.extend( DataTable.ext.type.detect, [
		...
		function ( d )
		{
			return _htmlNumeric( d ) ? 'jf-numeric' : null;
		},

		// HTML numeric, formatted
		function ( d )
		{
			return _htmlNumeric( d, true ) ? 'jf-numeric-fmt' : null;
		},

		// HTML (this is strict checking - there much be jf)
		function ( d )
		{
